...
class frontdashclass(QtWidgets.QMainWindow):
    ...
    def textbrowserwin(self):
        ...
        try:
            ...
            mes = self.ui.treeWidget.model().index(ase, 0)
            first_cell_selected = self.ui.treeWidget.model().data(mes, QtCore.Qt.DisplayRole)
            first_cell_selected2 = mkvmergingdict.get(first_cell_selected)
        


            a = threading.Thread(target=lambda :self.textexerun(first_cell_selected,first_cell_selected2))
            a.start()

    def process_num(self, process):
        return [p.info for p in psutil.process_iter(attrs=['pid', 'name']) if process in p.info['name']]

    def textexerun(self,ass,mkv):
        for i in self.process_num("textmenu.exe"):
            a = dict(i).get("pid")
            p = psutil.Process(a)
            p.terminate()
        subprocess.run([r"textmenu\textmenu",ass,mkv])
    ...
    def doingsomething(self,i,temp):
       ...
       # self.extracing = threading.Thread(target=lambda: self.extractingthread(forextract2))
        ...
        for assfilee in assfiles:
            ...
            if (str(mkvmergingdict.get(assfilee)) == None) or(str(mkvmergingdict.get(assfilee)) == "None"):
                assfile = assfilee
            else:
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QtGui.QIcon('ico.png'))
        self.showMaximized()
#        self.setWindowModality(QtCore.Qt.WindowModality)
        dummydata = QtWidgets.QTreeWidgetItem(["Dummy","Dummy"])
        self.ui.treeWidget.addTopLevelItem(dummydata)
        self.ui.treeWidget.setHeaderHidden(True)
        self.ui.treeWidget.hideColumn(0)
